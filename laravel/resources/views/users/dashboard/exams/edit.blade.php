<x-base title="Edit Ujian" main-class="container" no-main>
  <x-slot:head>
    <style>
      body {
        margin: 0;
      ...
      <div id="questions">
        @foreach(old('soal') ?? $questions as $i => $question)
          <div class="question-box" data-soal-idx="{{ $i }}">
            <label>
              Soal
              <textarea name="soal[{{ $i }}][soal]" rows="3" placeholder="..." @error("soal.".$i.".soal") aria-invalid="true" @enderror required>{{ $question['soal'] }}</textarea>
            </label>
            <div>
            ...
            <div class="btn-group">
              ...
              <button type="button" onclick="hapusJawaban(this.parentElement.previousElementSibling, this)" @if(count($question['jawaban']) <= 1) disabled @endif>Kurangi jawaban</button>
            </div>
